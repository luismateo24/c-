    ...
    try {
        ...
        if (!context.Roles.Any())
        {
            context.Roles.AddRange(
                new ErosMarketApi.Models.Role { Name = "Administrador" },
                new ErosMarketApi.Models.Role { Name = "Invitado" }
            );
            context.SaveChanges();
        }
        ...
        if (!context.Users.Any())
        {
            ...
            if (adminRole != null)
            {
                context.Users.Add(new ErosMarketApi.Models.User
                {
                    Username = "Admin",
                    Email = "admin@erosmarket.com",
                    PasswordHash = hash,
                    Role = adminRole,
                    Avatar = "https://i.pravatar.cc/150?u=admin" // Image URL for admin avatar
                });
                context.SaveChanges();
            }
        }
